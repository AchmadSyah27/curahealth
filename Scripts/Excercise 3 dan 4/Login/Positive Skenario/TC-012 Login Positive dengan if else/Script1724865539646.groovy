...
WebUI.sendKeys(findTestObject('Page_Login/textbox_username'), usernameSalah)
...
'Memastikan error text gagal login muncul'
errLabel = WebUI.verifyElementPresent(findTestObject('Page_Login/label_loginGagal'), 0)

'Melakukan screenshot gagal login'
WebUI.takeScreenshot('screenshot/loginGagal.png')

'Melakukan pengecekan login jika failed maka akan input ulang usernamenya'
if (errLabel == true) {
    'Melakukan input dengan username benar pada textbox username'
    WebUI.sendKeys(findTestObject('Page_Login/textbox_username'), username)
    ...
    'langsung diarahkan close browser'
    WebUI.closeBrowser()
}
